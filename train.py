...
def get_dataloader(dataset, batch_size =32, augment=True):
    xs, ys =[],[]
    np.random.seed()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_file", default="vaihingen.json", help="config file path")
    ...
    train_loader = get_dataloader(train_dataset, batch_size=PARAMS["batch_size"], augment=True)
    test_loader = get_dataloader(test_dataset, batch_size=PARAMS["batch_size"], augment=False)

    model = Net(train_dataset.num_classes, 3)
